...
class OgreXMLConverter:
    ...
    def convert_to_xml(self, input_file, output_dir=None):
        ...
        try:
            ...
            # Don't use check=True because OgreXMLConverter returns non-zero
            # for skeleton warnings
            result = subprocess.run(
                cmd, capture_output=True, text=True, creationflags=CREATE_NO_WINDOW
            )
            ...
            print("  Please specify path with --ogre-tools")
            return None
...
class OgreXMLToOBJ:
    ...
    def write_obj(self, output_file, mtl_file=None):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            have_normals = (
                len(self.normals) == len(self.vertices) and len(self.normals) > 0
            )
...
Examples:
  ...
  python MeshToObj.py mesh.mesh -o output.obj --ogre-tools /path/to/ogre/tools/
    ...
    parser.add_argument(
        "input", help="Input .mesh/.xml file or directory (with --batch)"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Output .obj file or directory"
    )
    parser.add_argument("--batch", action="store_true", help="Batch process directory")
    parser.add_argument("--ogre-tools", help="Path to Ogre command line tools")
    parser.add_argument(
        "--keep-xml", action="store_true", help="Keep intermediate XML files"
    )
    ...
    if args.batch:
        ...
        if xml_file:
            ...
            if cleanup_xml and not args.keep_xml and os.path.exists(xml_file):
                os.remove(xml_file)
                print("✓ Cleaned up temporary XML file")
        else:
