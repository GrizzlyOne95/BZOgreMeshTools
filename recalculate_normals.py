...
def recalculate_normals(xml_file_path):
    ...
    try:
        ...
        for submesh_idx, submesh in enumerate(submeshes):
            ...
            if geometry is None:
                print(f"    Warning: No geometry found for submesh {
                        submesh_idx + 1}")
                continue
            ...
            if vertex_buffer is None:
                print(f"    Warning: No vertex buffer found for submesh {
                        submesh_idx + 1}")
                continue

            # Check if normals are present
            if vertex_buffer.get("normals") != "true":
                print(f"    Warning: No normals in vertex buffer for submesh {
                        submesh_idx + 1}")
                continue
            ...
            if vertex_count == 0:
                print(f"    Warning: No positions found for submesh {
                        submesh_idx + 1}")
                continue
            ...
            if faces_elem is None:
                print(f"    Warning: No faces found for submesh {
                        submesh_idx + 1}")
                continue
            ...
            for face in faces:
                try:
                    ...
                    if (
                        v1_idx < vertex_count
                        and v2_idx < vertex_count
                        and v3_idx < vertex_count
                    ):
                        # Calculate face normal
                        face_normal = calculate_face_normal(
                            positions[v1_idx], positions[v2_idx], positions[v3_idx]
                        )
            ...
            for i, vertex in enumerate(vertices):
                if vertex_face_count[i] > 0:
                    ...
                    if normal_elem is not None:
                        # Check if significantly different (using a small
                        # epsilon)
                        ...
                        if (
                            abs(old_x - final_normal.x) > 0.0001
                            or abs(old_y - final_normal.y) > 0.0001
                            or abs(old_z - final_normal.z) > 0.0001
                        ):
        ...
        if any_changes_made:
            ...
            print(
                f"  Successfully recalculated normals for {total_vertices_processed} vertices"
            )
            return "CHANGED"
        else:
